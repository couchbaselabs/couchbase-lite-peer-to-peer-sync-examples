...
namespace P2PListSync
{
    ...
    public sealed class CoreApp : Application
    {
        ...
        internal static async Task LoadUserAllowListAsync()
        {
            using var stream = await FileSystem.OpenAppPackageFileAsync("userallowlist.json");
            using var reader = new StreamReader(stream);
            string userAllowList = await reader.ReadToEndAsync();
        ...
        public static void LoadAndInitDB()
        {
            ...
            if (!Database.Exists(DbName, DBPath)) {
                using (ZipArchive dbZip = new ZipArchive(FileSystem.OpenAppPackageFileAsync($"{DbName}.cblite2.zip").Result)) {
        ...
        internal static async Task LoadMauiAsset(string filename)
        {
