...
image::https://drive.google.com/file/d/1fM14NSvtrqcvQ7tWjNyhxNcvQaDKxrDE/view?usp=sharing[List App with p2p Sync][alt=peer to peer sync,width=600,height=480]

== App Installation

=== Downloading .zip

- Download the .zip project from link:https://drive.google.com/file/d/1fM14NSvtrqcvQ7tWjNyhxNcvQaDKxrDE/view?usp=sharing[here]
...
+
image::swift-login.png[List App Login Image][alt=app login screen,width=600,height=320]

== Exploring the App Project

- The xcode project comes pre-bundled with some resource files that we will examine here

image::swift-project-explorer.png[Project Explorer][alt=xcode project explorer,width=600,height=320]
...
<4> Configure the password authenticator callback function that authenticates the  username/password received from the client during replication setup. The list of valid users are configured in `userwhitelist.json` file bundled with the app
...
image::ios-passive-start-listener.gif[Passive Peer][alt=server websockets listener login screen,width=600,height=320]
...
image::ios-active-start-replicator.gif[Passive Peer][alt=p2p sync,width=600,height=320]
...
image::ios-sync.gif[Passive Peer][alt=server websockets listener login screen,width=600,height=320]
