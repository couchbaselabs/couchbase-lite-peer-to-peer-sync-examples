= Getting Started with Peer-to-Peer Sync on Java/Android
...
* Android Studio 4.1+
** Download a recent version from the link:https://developer.android.com/studio/[Android Developer]

* A Wi-Fi network over which peers can communicate
** You can also run your peers in multiple simulators. Unless you use real devices connected to a single local-area, the device discovery feature may not work
...
* Users log in and start websockets listeners for a couchbase lite database. Listeners are advertised on the local network using Android's Nearby.
