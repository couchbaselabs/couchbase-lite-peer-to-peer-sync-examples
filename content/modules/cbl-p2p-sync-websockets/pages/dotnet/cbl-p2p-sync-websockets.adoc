...
** NOTE: You may encounter issue when starting the listener with TLS on UWP. Please see <<Having issue start listener with TLS on UWP?>>
...
==== Having issue start listener with TLS on UWP?
* If start listener crash the UWP app, please uninstall the app first. On a 64-bit Windows, select "Debug" for Solution Configurations and "x64" for Solution Platforms on VS and Deploy the UWP sample app. After successfully deployed the app, you can run the app by clicking "Local Machine".