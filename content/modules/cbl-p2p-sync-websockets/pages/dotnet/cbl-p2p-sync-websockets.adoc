...
image::xamarin-passive-start-listener.gif[alt=server websockets listener login screen,width=600,height=320]

== Active Peer or Client
We will walk through the steps of using the app in active peer mode

=== Discovering Listeners
In the app, we use UDP Type Socket to listen on port 15000 for listener Note: Port 15000 is used by UDP Type Socket, not used by the websocket listener. Couchbase Lite choose the port when a websocket listener is generated.
...
image::xamarin-sync.gif[alt=server websockets listener login screen,width=600,height=320]
