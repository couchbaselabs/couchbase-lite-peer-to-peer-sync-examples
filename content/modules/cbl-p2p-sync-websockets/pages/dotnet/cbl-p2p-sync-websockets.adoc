...
* Open the *ReplicatorItem.cs* file and locate the `StopReplicator()` method. If you have been using Couchbase Lite to sync data with Sync Gateway, this code should seem very familiar. In this function, we remove any listeners attached to the replicator and stop it. You can restart the replicator again in the same `ExecuteStartReplicatorCommand()` method
